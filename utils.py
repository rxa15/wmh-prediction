        ...
        for scan_pair, paths in folder_map.items():
            ...
            for f in os.listdir(flair_folder):
                ...
                if wmh_folder is not None:
                    # 1) Get the base filename prefix (e.g., "LBC360001_1")
                    if "_FLAIR" in fname:
                        flair_base = fname.split("_FLAIR")[0]
                    else:
                        parts = fname.split("_")
                        if len(parts) >= 3:
                            flair_base = "_".join(parts[:2])
                        else:
                            flair_base = fname.split(".nii")[0]

                    # 2) Look for a matching WMH file in the WMH folder
...
def _match_channels(x: torch.Tensor, desired_channels: int) -> torch.Tensor:
    """
    Match tensor channel count to desired_channels by truncating or zero-padding.
    Keeps spatial dims unchanged.
    """
    if x.shape[1] == desired_channels:
        return x
    if x.shape[1] > desired_channels:
        return x[:, :desired_channels]
    missing = desired_channels - x.shape[1]
    zeros = torch.zeros(
        (x.shape[0], missing, x.shape[2], x.shape[3]),
        dtype=x.dtype,
        device=x.device,
    )
...
def evaluate_and_visualize_tasks(model_path, source_loader, gt_loaders, device, original_scans_dir, in_channels, out_channels=1, results_dir="results"):
    """Evaluates a model on specific tasks and saves predictions as 3D NIfTI files."""
    print(f"\n--- Evaluating: {os.path.basename(model_path)} ---")

    model = ImageFlowNetODE_FlexibleOutput(device=device, in_channels=in_channels, out_channels=out_channels, ode_location='bottleneck', contrastive=True).to(device)
...
def load_folds_from_csv(fold_csv_path):
    ...
    # Support both "fold" and "split_id" column names
    fold_column = "split_id" if "split_id" in df.columns else "fold"
...
def _normalize_patient_id(pid) -> str:
    """
    Normalize patient identifiers to the 4-digit numeric ID used across this repo.
...
def load_patient_split_map(split_csv_path: str) -> dict:
    """
    Load patient->split_id mapping from a CSV.

    Supports either:
      - columns: patient_ID, split_id
      - columns: patient_ID, fold
    """
    df = pd.read_csv(split_csv_path)
    if "patient_ID" not in df.columns:
        raise ValueError(f"Expected column 'patient_ID' in {split_csv_path}")

    split_col = "split_id" if "split_id" in df.columns else ("fold" if "fold" in df.columns else None)
    if split_col is None:
        raise ValueError(f"Expected column 'split_id' or 'fold' in {split_csv_path}")

    patient_to_split = {}
    for _, row in df.iterrows():
        pid = _normalize_patient_id(row["patient_ID"])
        patient_to_split[pid] = int(row[split_col])
    return patient_to_split


def _patient_ids_for_splits(patient_to_split: dict, splits) -> set:
    splits = {int(s) for s in splits}
    return {pid for pid, s in patient_to_split.items() if int(s) in splits}


class EvolutionSegmentationDataset(Dataset):
    """
    Wraps a FLAIREvolutionDataset item (which returns 2-ch [FLAIR, WMH]) into a
    segmentation sample with keys {"flair","mask"}.

    Intended for training SwinUNETR directly from original data (before Stage 1),
    while still honoring patient splits.
    """

    def __init__(self, base_dataset: "FLAIREvolutionDataset", indices):
    ...
    def __getitem__(self, idx):
        ...
        if source.ndim != 3 or source.shape[0] < 2:
            raise ValueError(
                "EvolutionSegmentationDataset expects base 'source' with >=2 channels [FLAIR, WMH]. "
                f"Got shape={tuple(source.shape)}. Ensure base_dataset was created with use_wmh=True."
            )
        flair = source[0:1]
        mask = source[1:2]
        return {
            "flair": flair,
            "mask": mask,
            "patient_id": _normalize_patient_id(item.get("patient_id", "")),
...
def train_swinunetr_4fold_from_csv(
    *,
    root_dir: str,
    split_csv_path: str,
    ...
    lr: float = 1e-4,
    device=None,
    models_dir: str = "swinunetr_models",
    val_offset: int = 1,
    require_wmh_presence: bool = True,
):
    """
    Train 4 independent SwinUNETR models with consistent (train/val/test) splits from 4fold_split.csv.

    For each outer fold:
      - test_split = i
      - val_split  = i shifted by val_offset (default: next split)
      - train_splits = remaining two splits

    Saves one checkpoint per outer fold into models_dir and returns a summary DataFrame.
    """
    ...
    patient_to_split = load_patient_split_map(split_csv_path)
    all_splits = sorted(set(patient_to_split.values()))
    if len(all_splits) != 4:
        raise ValueError(f"Expected exactly 4 splits in {split_csv_path}, got {all_splits}")

    # Build ONE base dataset over the chosen scan (identity pair) with WMH enabled.
    ...
    # Precompute indices per split for speed.
    ...
    for test_split in all_splits:
        ...
        train_splits = [s for s in all_splits if s not in (test_split, val_split)]

        train_indices = []
        for s in train_splits:
            train_indices.extend(split_to_indices[s])
        val_indices = split_to_indices[val_split]
        test_indices = split_to_indices[test_split]

        train_ds = EvolutionSegmentationDataset(base_dataset, train_indices)
        val_ds = EvolutionSegmentationDataset(base_dataset, val_indices)
        ...
        model.eval()
        test_dice = eval_segmentation(model, test_loader, device)

        rows.append(
            {
                "test_split": test_split,
                "val_split": val_split,
                "train_splits": ",".join(map(str, train_splits)),
                "num_train_slices": len(train_ds),
                "num_val_slices": len(val_ds),
                "num_test_slices": len(test_ds),
